...
This is an implementation of "Clustering the Latent/Reconstructed Data point By Auto-Encoder" algorithm for
...
class AutoEncoder(tf.keras.Model):
    ...
    def call(self, x):
        ...
        decoded = self.decoder(coded)  # reconstructed
...
def run_cluster_latents(X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for X_vl, _ in val_ds:
            ...
            val_X_hat = np.c_[val_X_hat, decodes_.numpy().T]  # concatenating reconstructed data points
    ...
    print("K-means: ", metrics.roc_auc_score(y_val, kmeans_val_labels, average='weighted'),
          "Agg: ", metrics.roc_auc_score(y_val, agg_val_labels, average='weighted'),
          "GMM: ", metrics.roc_auc_score(y_val, gmm_val_labels, average='weighted'),
          )
    ...
    X_test_hat = autoencoder.decoder(Z_test).numpy()  # reconstructed data points
    ...
    try:
        ...
        gmm_X_test_labels = np.repeat(X_test.shape[0], 0)
    ...
    try:
        ...
        gmm_X_test_hat_labels = np.repeat(X_test.shape[0], 0)
    ...
    try:
        ...
        gmm_Z_test_labels = np.repeat(X_test.shape[0], 0)
...
if __name__ == '__main__':
    ...
    if run == 1:
        ...
        if setting_ != 'all':
            ...
            with open(os.path.join('AE-C_computation', "kmeans_ms_" +
                                                        name + "-" + pp + "-" + setting_ + ".pickle"), 'wb') as fp:
                pickle.dump(kmeans_ms, fp)

            # Saving Agglomerative results
            with open(os.path.join('AE-C_computation', "agg_ms_" +
                                                        name + "-" + pp + "-" + setting_ + ".pickle"), 'wb') as fp:
                pickle.dump(agg_ms, fp)

            # Saving Gaussian Mixture Model
            with open(os.path.join('AE-C_computation', "gmm_ms_" +
                                                        name + "-" + pp + "-" + setting_ + ".pickle"), 'wb') as fp:
    ...
    if run == 0:
        ...
        with open(os.path.join('AE-C_computation', "kmeans_ms_" + name + "-" + pp + ".pickle"), 'rb') as fp:
            kmeans_ms = pickle.load(fp)

        with open(os.path.join('AE-C_computation', "agg_ms_" + name + "-" + pp + ".pickle"), 'rb') as fp:
            agg_ms = pickle.load(fp)

        with open(os.path.join('AE-C_computation', "gmm_ms_" + name + "-" + pp + ".pickle"), 'rb') as fp:
