...
This is an implementation of "Energy-Based One Plus Epsilon" algorithm for ML4DC  studies on CERN data set.
...
def crude_monte_carlo(n_pos, original_dim, model, supp_min, supp_max, num_samples=100, ):

    sum_samples = 0

    for _ in range(num_samples):

        x_pseudo = tf.random.uniform(shape=[n_pos, original_dim],
...
def loss_bf(model, supp_min, supp_max, x_batch, x_pos, x_neg,):
    ...
    loss_pseudo = crude_monte_carlo(n_pos=n_pos, original_dim=v, model=model,
                                    supp_min=supp_min, supp_max=supp_max, num_samples=100)
...
if __name__ == '__main__':
    ...
    if run == 1:
        ...
        with open(os.path.join('FNN_computation', NAME + name + "-" + pp +
